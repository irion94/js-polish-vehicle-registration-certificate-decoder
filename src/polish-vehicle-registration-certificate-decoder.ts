...
const polish_vehicle_registration_certificate_new_format_data_1 = require("./polish-vehicle-registration-certificate-new-format-data");
const polish_vehicle_registration_certificate_old_format_data_1 = require("./polish-vehicle-registration-certificate-old-format-data");
const nrv2e_decompress_1 = tslib_1.__importDefault(require("nrv2e-decompress"));
...
const fieldsDefinition = {
    ...
    seriaDr: { name: 'SERIA DR', description: 'seria DR', newFormatIndex: 1, oldFormatIndex: 0 },
    kodTerytorialnyUrzedu: { name: 'nd.', description: 'kod terytorium urzędu', newFormatIndex: 2 },
    organWydajacy: { name: 'ORGAN WYDAJĄCY', description: 'organ wydający', newFormatIndex: 3, oldFormatIndex: 1, lines: 4 },
    ...
    rodzajPojazdu: { name: 'RODZAJ POJAZDU', description: 'rodzaj pojazdu', newFormatIndex: 54, oldFormatIndex: 39 },
    przeznaczenie: { name: 'PRZEZNACZENIE', description: 'przeznaczenie', newFormatIndex: 55, oldFormatIndex: 40 },
    rokProdukcji: { name: 'ROK PRODUKCJI', description: 'rok produkcji', newFormatIndex: 56, oldFormatIndex: 41 },
    ...
    nrKartyPojazdu: { name: 'NR KARTY POJAZDU', description: 'numer karty pojazdu', newFormatIndex: 59, oldFormatIndex: 44 },
    ...
    phoneNumber:{name: 'NUMER TELEFONU', description: 'numer telefonu', newFormatIndex: 66},
    email:{name: 'EMAIL', description: 'e-mail', newFormatIndex: 66}
};
class PolishVehicleRegistrationCertificateDecoder {
    constructor(b64Input) {
        ...
        this.data = new polish_vehicle_registration_certificate_new_format_data_1.PolishVehicleRegistrationCertificateNewFormatData();
        for (const key of Object.keys(fieldsDefinition)) {
            const field = fieldsDefinition[key];
                const newDataField = { name: field.name, description: field.description };
                const index = field.newFormatIndex;
                if (index !== undefined) {
                    if (field.lines) {
